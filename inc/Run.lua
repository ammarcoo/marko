...
marko = Token:match("(%d+)")
redis:set(marko..":VERSION",1)
redis:set(marko..":SUDO_ID:",SUDO_USER)
redis:set(marko..":DataCenter:",'German')
redis:set(marko..":UserNameBot:",BOT_User)
redis:set(marko..":NameBot:",BOT_NAME)
redis:hset(marko..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3marko_INSTALL","Yes")
info = {}
...
marko = Token:match("(%d+)")
our_id = tonumber(marko)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(marko..":UserNameBot:")
SUDO_ID = tonumber(redis:get(marko..":SUDO_ID:"))
SUDO_USER = redis:hgetall(marko..'username:'..SUDO_ID).username
version = redis:get(marko..":VERSION")
DataCenter = redis:get(marko..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..marko..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(marko..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(marko..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(marko..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(marko..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور '
msg.TheRank = 'مطور البوت '
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(marko..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(marko..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء '
msg.TheRank = 'المنشىء '
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(marko..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير ' 
msg.TheRank = 'مدير البوت ' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(marko..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن '
msg.TheRank = 'ادمن في البوت '
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(marko..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(marko..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(marko..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(marko..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(marko..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(marko..':WELCOME_BOT'),[[•~مـرحبآ آنآ بوت آسـمـي ]]..redis:get(marko..':NameBot:')..[[•~آختصـآصـي حمـآيه‌‏ آلمـجمـوعآت
...
if msg.GroupActive and redis:get(marko..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(marko..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(marko..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(marko..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.marko then 
if msg.text and ScriptFile.imarko then
for k, marko in pairs(ScriptFile.marko) do
local SearchText = msg.text:match(marko)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.imarko(msg,{msg.text:match(marko)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",marko.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dmarko then
if ScriptFile.dmarko(msg) == false then
return false
end
...
if Plug.marko then 
if msg.text and Plug.imarko then
for k, marko in pairs(Plug.marko) do
local SearchText = msg.text:match(marko)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(marko..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(marko..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.imarko(msg,{msg.text:match(marko)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",marko.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source marko")
end 

end
else
print("The File Script.lua Not Runing in The Source marko")
...
function tdcli_update_callback(data)
	...
	redis:srem(marko..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/ammarcoo/marko/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/ammarcoo/marko/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/ammarcoo/marko/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/ammarcoo/marko/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/ammarcoo/marko/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/ammarcoo/marko/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(marko..":SUDO_ID:")
redis:hset(marko..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(marko..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(marko..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(marko..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(marko..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(marko..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(marko..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(marko..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(marko..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(marko..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(marko..'group:add'..data.chat_.id_) then
	redis:set(marko..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(marko..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(marko..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(marko..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	marko..'group:name'..k,IDS.Title,
	marko..'num_msg_marko'..k,5,
	marko..'group:add'..k,true,
	marko..'lock_link'..k,true,
	marko..'lock_spam'..k,true,
	marko..'lock_webpage'..k,true,
	marko..'lock_markdown'..k,true,
	marko..'lock_flood'..k,true,
	marko..'lock_bots'..k,true,
	marko..'mute_forward'..k,true,
	marko..'mute_contact'..k,true,
	marko..'mute_document'..k,true,
	marko..'mute_inline'..k,true,
	marko..'lock_username'..k,true,
	marko..'replay'..k,true
	)
	redis:sadd(marko..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(marko..'username:'..ID,'username',user)
	redis:sadd(marko..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(marko..'username:'..ID,'username',user)
	redis:sadd(marko..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(marko..'username:'..ID,'username',user)
	redis:sadd(marko..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'•~** تم رفع آلنسـخه‏‏ آلآحتيآطـيه\n•~** حآليآ عدد مـجمـوعآتك هيه *'..redis:scard(marko..'group:ids')..'* ®️\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(marko..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(marko..'username:'..data.user_.id_).username
	...
	redis:hset(marko..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
