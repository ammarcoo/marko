...
─────▀██▀─────
┊amrkoo ‿ @XXUUU
تم تطوير وبرمجة السورس من قبل مطورين سورس عماركو
┊@XXUUU ‿ @XXUUU
#-------------------------------------------------------------------
]]
...
amrkoo = Token:match("(%d+)")
redis:set(amrkoo..":VERSION",1)
redis:set(amrkoo..":SUDO_ID:",SUDO_USER)
redis:set(amrkoo..":DataCenter:",'German')
redis:set(amrkoo..":UserNameBot:",BOT_User)
redis:set(amrkoo..":NameBot:",BOT_NAME)
redis:hset(amrkoo..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3amrkoo_INSTALL","Yes")
info = {}
...
amrkoo = Token:match("(%d+)")
our_id = tonumber(amrkoo)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(amrkoo..":UserNameBot:")
SUDO_ID = tonumber(redis:get(amrkoo..":SUDO_ID:"))
SUDO_USER = redis:hgetall(amrkoo..'username:'..SUDO_ID).username
version = redis:get(amrkoo..":VERSION")
DataCenter = redis:get(amrkoo..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..amrkoo..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(amrkoo..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(amrkoo..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(amrkoo..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(amrkoo..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور '
msg.TheRank = 'مطور البوت '
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(amrkoo..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(amrkoo..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء '
msg.TheRank = 'المنشىء '
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(amrkoo..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير ' 
msg.TheRank = 'مدير البوت ' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(amrkoo..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن '
msg.TheRank = 'ادمن في البوت '
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(amrkoo..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(amrkoo..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(amrkoo..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(amrkoo..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(amrkoo..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(amrkoo..':WELCOME_BOT'),[[•~مـرحبآ آنآ بوت آسـمـي ]]..redis:get(amrkoo..':NameBot:')..[[•~آختصـآصـي حمـآيه‌‏ آلمـجمـوعآت
...
if msg.GroupActive and redis:get(amrkoo..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(amrkoo..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(amrkoo..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(amrkoo..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.amrkoo then 
if msg.text and ScriptFile.iamrkoo then
for k, amrkoo in pairs(ScriptFile.amrkoo) do
local SearchText = msg.text:match(amrkoo)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iamrkoo(msg,{msg.text:match(amrkoo)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",amrkoo.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.damrkoo then
if ScriptFile.damrkoo(msg) == false then
return false
end
...
if Plug.amrkoo then 
if msg.text and Plug.iamrkoo then
for k, amrkoo in pairs(Plug.amrkoo) do
local SearchText = msg.text:match(amrkoo)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(amrkoo..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(amrkoo..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iamrkoo(msg,{msg.text:match(amrkoo)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",amrkoo.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.damrkoo then
Plug.damrkoo(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source amrkoo")
end 

end
else
print("The File Script.lua Not Runing in The Source amrkoo")
...
function tdcli_update_callback(data)
	...
	redis:srem(amrkoo..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/ammarcoo/amrkoo/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/ammarcoo/amrkoo/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/ammarcoo/amrkoo/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/ammarcoo/amrkoo/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/ammarcoo/amrkoo/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/ammarcoo/amrkoo/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(amrkoo..":SUDO_ID:")
redis:hset(amrkoo..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(amrkoo..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(amrkoo..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(amrkoo..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(amrkoo..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(amrkoo..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(amrkoo..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(amrkoo..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(amrkoo..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(amrkoo..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(amrkoo..'group:add'..data.chat_.id_) then
	redis:set(amrkoo..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(amrkoo..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(amrkoo..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(amrkoo..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	amrkoo..'group:name'..k,IDS.Title,
	amrkoo..'num_msg_amrkoo'..k,5,
	amrkoo..'group:add'..k,true,
	amrkoo..'lock_link'..k,true,
	amrkoo..'lock_spam'..k,true,
	amrkoo..'lock_webpage'..k,true,
	amrkoo..'lock_markdown'..k,true,
	amrkoo..'lock_flood'..k,true,
	amrkoo..'lock_bots'..k,true,
	amrkoo..'mute_forward'..k,true,
	amrkoo..'mute_contact'..k,true,
	amrkoo..'mute_document'..k,true,
	amrkoo..'mute_inline'..k,true,
	amrkoo..'lock_username'..k,true,
	amrkoo..'replay'..k,true
	)
	redis:sadd(amrkoo..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(amrkoo..'username:'..ID,'username',user)
	redis:sadd(amrkoo..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(amrkoo..'username:'..ID,'username',user)
	redis:sadd(amrkoo..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(amrkoo..'username:'..ID,'username',user)
	redis:sadd(amrkoo..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'•~** تم رفع آلنسـخه‏‏ آلآحتيآطـيه\n•~** حآليآ عدد مـجمـوعآتك هيه *'..redis:scard(amrkoo..'group:ids')..'* ®️\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(amrkoo..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(amrkoo..'username:'..data.user_.id_).username
	...
	redis:hset(amrkoo..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
