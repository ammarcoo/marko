...
----------------------{ Get Name Bot }-----------------------------
Bot_Name = redis:get(amrko..":NameBot:")
...
var = 'مطور البوت' 
elseif redis:sismember(amrko..':KARA_BOT:'..ChatID,UserID) then
var = ' المنشىء الاساسي' 
...
EnterCmd = true
local NumberFlood = tonumber(redis:get(amrko..':Flood_Spam:'..msg.sender_user_id_..':msgs') or 0)
if NumberFlood > 3 then 
redis:setex(amrko..':Flood_Spam:'..msg.sender_user_id_..':Stoped',10,true)
EnterCmd = false
elseif redis:get(amrko..':Flood_Spam:'..msg.sender_user_id_..':Stoped') then
EnterCmd = false
else
redis:setex(amrko..':Flood_Spam:'..msg.sender_user_id_..':msgs',3,NumberFlood+1)
end
return EnterCmd
end
...
local creators = redis:smembers(amrko..':MONSHA_BOT:'..GroupS)
if #creators ~= 0 then
...
local creator = redis:smembers(amrko..':KARA_BOT:'..GroupS)
if #creator ~= 0 then
...
redis:hset(amrko..'username:'..data.id_,'username',ResolveUser(data))
redis:sadd(amrko..'admins:'..msg.chat_id_,data.id_)
end,nil)
end
end
if NumAdmin == 0 then 
...
if redis:get(amrko..'group:add'..msg.chat_id_) then  return '• **المجموعة تم تفعيلها مُسبقاً . ' end
...
redis:setex(amrko..":ismember:"..msg.sender_user_id_,1500,true)
...
local lock_service = redis:get(amrko..'lock_service')
...
GetUserID(v.user_id_,function(arg,data)
redis:hset(amrko..'username:'..data.id_, 'username', ResolveUser(data))
redis:sadd(amrko..':KARA_BOT:'..msg.chat_id_,data.id_)
end)
...
redis:mset(
amrko..'group:add'..msg.chat_id_,true,
amrko..'lock_link'..msg.chat_id_,true, 
amrko..'lock_id'..msg.chat_id_,true,
amrko..'lock_spam'..msg.chat_id_,true,
amrko..'lock_webpage'..msg.chat_id_,true,
amrko..'lock_markdown'..msg.chat_id_,true,
amrko..'lock_flood'..msg.chat_id_,true,
...
amrko..'mute_keyboard'..msg.chat_id_,true,
amrko..'mute_game'..msg.chat_id_,true,
amrko..'mute_inline'..msg.chat_id_,true,
amrko..'lock_username'..msg.chat_id_,true,
amrko..'num_msg_amrko'..msg.chat_id_,5, 
amrko..'replay'..msg.chat_id_,true
)

redis:sadd(amrko..'group:ids',msg.chat_id_) 
redis:sadd(amrko..'mtwr_count'..msg.sender_user_id_,msg.chat_id_)

GroupTitle(msg.chat_id_,function(arg,data)
local NameGroup = data.title_
redis:set(amrko..'group:name'..msg.chat_id_,NameGroup)
...
redis:set(amrko..'linkGroup'..msg.chat_id_,Gp_Link)
...
if not redis:get(amrko..'group:add'..msg.chat_id_) then return '** المجموعه تم تعطيلها مُسبقاً .' end  
...
if redis:sismember(amrko..':KARA_BOT:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'• العضو » ❪ '..USERNAME..' ❫\n• الايدي » ❪ '..UserID..' ❫\n• انه بالتأكيد منشىء اساسي \n✓️',17,USERCAR) 
end
redis:hset(amrko..'username:'..UserID,'username',Resolv)
redis:sadd(amrko..':KARA_BOT:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'• العضو » ❪ '..USERNAME..' ❫\n• الايدي » ❪ '..UserID..' ❫\n• تمت ترقيته ليصبح منشىء اساسي \n✓️',17,USERCAR) 
end

if cmd == "remkara" then
if not redis:sismember(amrko..':KARA_BOT:'..ChatID,UserID) then
return SendMention(ChatID,UserID,MsgID,'• العضو » ❪ '..USERNAME..' ❫\n• الايدي » ❪ '..UserID..' ❫\n• انه بالتأكيد ليس منشىء اساسي\n✓️',17,USERCAR) 
end
redis:srem(amrko..':KARA_BOT:'..ChatID,UserID)
...
if cmd == "unban" then
if not redis:sismember(amrko..'banned:'..ChatID,UserID) then 
...
if redis:sismember(amrko..'is_silent_users:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'• العضو » ❪ '..USERNAME..' ❫\n• الايدي » ❪ '..UserID..' ❫\n• تم بالتأكيد كتمه  \n',17,USERCAR) 
end
redis:hset(amrko..'username:'..UserID, 'username', Resolv)
...
if not redis:sismember(amrko..'is_silent_users:'..ChatID,UserID) then 
...
if redis:sismember(amrko..'banned:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'• **العضو » ❪ '..UserName..' ❫\n• **الايدي » ❪ `'..UserID..'` ❫\n• **تم بالتأكيد حظره \n') 
end
kick_user(UserID, ChatID,function(arg,data)
if data.ID == "Error" and data.code_ == 400 then
...
return sendMsg(ChatID,MsgID,"• **لا يمكنك طرد المطور\n") 
elseif
redis:sismember(amrko..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"• **لا يمكنك طرد المنشئ\n") 
elseif redis:sismember(amrko..'owners:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"• **لا يمكنك طرد المدير\n") 
elseif redis:sismember(amrko..'admins:'..ChatID,UserID) then 
...
if cmd == "dn_sudo" then
if not redis:sismember(amrko..':SUDO_BOT:',UserID) then return sendMsg(ChatID,MsgID,'• **العضو » '..USERNAME..' \n• ** الايدي » (`'..UserID..'`)\n• **انه بالتأكيد ليس مطور \n') end
redis:srem(amrko..':SUDO_BOT:',UserID) 
return SendMention(ChatID,UserID,MsgID,'• العضو » ❪ '..USERNAME..' ❫\n• الايدي » ❪ '..UserID..' ❫\n• تم تنزيله من المطور \n',17,USERCAR) 
end
...
if redis:get(amrko..'welcome:get'..msg.chat_id_) then welcomex = "❪ مفعل ❫" else welcomex = "❪ معطل ❫" end
if redis:get(amrko..'lock_id'..msg.chat_id_) then lock_idx = "❪ مفعل ❫" else lock_idx = "❪ معطل ❫" end
if redis:get(amrko..'replay'..msg.chat_id_)  then replayx  = "❪ مفعل ❫" else replayx  = "❪ معطل ❫" end
if redis:get(amrko..'lock_woring'..msg.chat_id_) then lock_woringx = "❪ مفعل ❫" else lock_woringx  = "❪ معطل ❫" end
...
.."\n• التاك «» "..(redis:get(amrko..'lock_tag'..msg.chat_id_) or 'false')
...
.."\n• الماركدوان «» "..(redis:get(amrko..'lock_markdown'..msg.chat_id_) or 'false')
.."\n• بالطرد البوتات «» "..(redis:get(amrko..'lock_bots_by_kick'..msg.chat_id_) or 'false')
.."\n• البوتات «» "..(redis:get(amrko..'lock_bots'..msg.chat_id_) or 'false')
.."\n• عدد التكرار «» "..(redis:get(amrko..'num_msg_amrko'..msg.chat_id_) or 'false')
...
.."\n• الانلاين «» "..(redis:get(amrko..'mute_inline'..msg.chat_id_) or 'false')
...
.."\n• البصمات «» "..(redis:get(amrko..'mute_audio'..msg.chat_id_) or 'false')
.."\n• الصوت «» "..(redis:get(amrko..'mute_voice'..msg.chat_id_) or 'false')
